...
- Ex. SQLite, MySQL, Supabase(PostgreSQL), Oracle
  - SQLite (클라이언트 수백명까진 커버 가능)
    - 경량DB; 임베디드에 도입하기 좋음 (주로 **로깅용**으로 사용됨)
    - **SBC**나 모바일쪽에 들어감
...
- if. 부서의 세부 분류 속성이 복수인 경우?
    ...
    - 이걸 **외래키(FK)**라고 하고, 테이블 식별자를 PK(공용키)라고 한다
    - 같은 Row 라면 반드시 같은 속성과 타입을 가져야하는 엄격함이 있음 => NoSQL 도입 이유
...
-- 단, user 라는 키워드는 예약되어 있기 때문에 쓰지 말 것
CREATE TABLE member (
  -- AUTOINCREMENT는 PK(int)가 자동 증가
  -- 단, 누락이 발생한 경우 빠진 부분을 채워 넣어주지 않음
  -- [Item] [Type]; VARCHAR(50), CHAR(50)
...
```sql
-- 조건부로 가져오기
-- 예약어 처리를 막기 위해 대괄호로 감싸준다.
SELECT * FROM 테이블명 WHERE [name] LIKE '%love%';
...
UPDATE member
  -- SET 뒤에 세미콜론 실수 주의 (가독성을 위한 들여쓰기임) 
